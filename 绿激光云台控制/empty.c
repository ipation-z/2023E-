...
static void Handle_UART_Packet(UART_Packet *pkt) {
    ...
    switch (pkt->mode) {
        ...
        case 0x02:
            ...
            if (g_ctrl.state == SYS_TRACKING_REL ||g_ctrl.state == SYS_TRACKING_REL_2) {
                g_ctrl.new_data  = true;
            }
            break;
...
int main(void) {
    ...
    //PB8设置为下拉
    DL_GPIO_clearPins(GPIO_WARN_PORT,GPIO_WARN_PIN_GND_PIN);
    DL_GPIO_setPins(GPIO_WARN_PORT,GPIO_WARN_PIN_BUZZER_PIN);
    while (1) {
        ...
        if (++display_cnt >= 5000) {
            ...
            const char *state_str[] = {
                ...
                // [SYS_ARRIVED]      = "ARRIVED",
                // [SYS_RESET]        = "RESET  "
                [SYS_TRACKING_REL_2] = "TRK_REL_2",
            };

            // 修改判断范围，仅针对保留的两个状态
            if (g_ctrl.state == SYS_IDLE || g_ctrl.state == SYS_TRACKING_REL || g_ctrl.state == SYS_TRACKING_REL_2) {
                tft180_printf(0, 0, "Mode: %s", state_str[g_ctrl.state]);
            } else {
            ...
            switch (g_ctrl.state) 
            {
                ...
                //     tft180_printf(0, 32, "Cur X: %-5d      ", g_ctrl.current.x); //当前坐标
                //     tft180_printf(0, 48, "Cur Y: %-5d      ", g_ctrl.current.y);
                //     tft180_printf(0, 64, "StepX: %-5d      ", g_manual_step_x); //手动记步
                //     tft180_printf(0, 80, "StepY: %-5d      ", g_manual_step_y); 
                ...
                case SYS_TRACKING_REL_2:
                    // 【相对追踪模式】(第二题) 重点显示：摄像头传来的相对误差
                    tft180_printf(0, 16, "Cur: %3d,%3d", g_ctrl.current.x, g_ctrl.current.y);
                    tft180_printf(0, 32, "Rel Err:%3d,%3d", (int)g_ctrl.rel_err_x, (int)g_ctrl.rel_err_y);
                    tft180_printf(0, 48, "                  "); 
                    tft180_printf(0, 64, "                  "); 
                    tft180_printf(0, 80, "                  "); 
                    tft180_printf(0, 6*16, "                  "); 
