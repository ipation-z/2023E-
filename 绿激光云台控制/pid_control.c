...
void PID_Handle_SW1_StateTransform(void) {
    ...
    switch (current_state) {
        ...
        case SYS_TRACKING_REL:
            g_ctrl.state = SYS_TRACKING_REL_2;
            break;
...
static void Drive_Axes_Synchronized(float out_x, float out_y) {
    ...
    uint32_t BASE_FREQ = 180;  // 建议范围 400 ~ 800，根据电机实际承载能力微调

    // ================= 【配合更小的细分，频率要加快】 =================
    if (g_ctrl.state == SYS_TRACKING_REL) {
        BASE_FREQ = BASE_FREQ * 3; //效果非常好
    }

    if (g_ctrl.state == SYS_TRACKING_REL_2) {
        BASE_FREQ = BASE_FREQ * 0.45; //
    }
    // ==================================================================================
...
/**
 * @brief 判断是否到达死区范围内并控制 LED 与蜂鸣器
 * @param err_x 当前 X 轴误差 (像素)
 * @param err_y 当前 Y 轴误差 (像素)
 */
void Indicator_Update(float err_x, float err_y) {
    // 获取当前模式下的死区值 (复用你在 pid_control.h 中定义的宏)
    int32_t deadzone = 30; //实测得到

    // 判断误差是否在死区范围内 (注意要加绝对值，否则负方向大误差也会被判定为符合)
    if (/*abs((int)err_x) <= deadzone && abs((int)err_y) <= deadzone*/sqrtf(err_x*err_x+err_y*err_y)<= deadzone) {
        LED_ON();       // 误差达标，LED 亮
        BUZZER_ON();    // IO 拉低，蜂鸣器响
    } else {
        LED_OFF();      // 误差未达标，LED 灭
        BUZZER_OFF();   // IO 拉高，蜂鸣器停止
    }
}